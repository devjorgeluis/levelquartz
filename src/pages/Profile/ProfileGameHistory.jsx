...
const ProfileGameHistory = () => {
    ...
    const fetchHistory = () => {
        ...
        queryParams = new URLSearchParams({
            ...
            ...(from && { date_from: formatDateForAPI(from) }),
            ...(to && { date_to: formatDateForAPI(to) }),
            type: "slot"
        }).toString();
    ...
    }, [from, to, period, pagination.currentPage]);
            ...
            <div className="tb--account-body tb--profile-scroll tl_gh">
                ...
                <div className="tb--history tb--empty-history">
                    <form name="myProfileForm" className="myProfileForm tb--game-history_tbl cashier-form-wrapper tb--recent-nodata" onSubmit={(event) => event.preventDefault()}>
                        <div className="transaction-header game-history-filter">
                            <div className="game-history-filter__field game-history-filter__period" ref={periodRef}>
                                <span className="game-history-filter__label" id="period-label">Período</span>
                                <button
                                    type="button"
                                    className={`game-history-filter__select ${isPeriodOpen ? "is-open" : ""}`}
                                ...
                                >
                                    ...
                                    <i className="digi_icon-arrow_down" aria-hidden="true" />
                                </button>

                                {isPeriodOpen && (
                                    <ul className="game-history-filter__menu" role="listbox" aria-label="Período">
                                        {periodOptions.map((option) => (
                                            <li
                                                key={option.value}
                                                className={period.value === option.value ? "is-selected" : ""}
                                                role="option"
                                                aria-selected={period.value === option.value}
                            ...
                            <label className="game-history-filter__field">
                                <span className="game-history-filter__label">De</span>
                                <span className="game-history-filter__date">
                                    <span>{formatDisplayDate(from)}</span>
                                    <i className="digi_icon-calendar" aria-hidden="true" />
                                    <input type="date" value={from} onChange={(event) => setFrom(event.target.value)} aria-label="De" />
                                </span>
                            </label>

                            <label className="game-history-filter__field">
                                <span className="game-history-filter__label">A</span>
                                <span className="game-history-filter__date">
                                    <span>{formatDisplayDate(to)}</span>
                                    <i className="digi_icon-calendar" aria-hidden="true" />
                                    <input type="date" value={to} onChange={(event) => setTo(event.target.value)} aria-label="A" />
                                </span>
                            </label>

                            <button type="submit" className="game-history-filter__submit" onClick={() => fetchHistory()}>Ver detalles</button>
                        </div>
                        <div className="tb--table-container tb--tabel-empty">
                            <div className="tb--home-game-heading tb--flex tb--align-center tb--ph-20 tb--mobile-hide">
                                <div className="id tb--acc22 tb--ns">Identificación</div>
                                <div className="dt tb--acc22 tb--ns">Fecha</div>
                                <div className="gn tb--acc22 tb--ns">Juego</div>
                                <div className="wg tb--acc22 tb--ns">Apuesta</div>
                                <div className="win tb--acc12 tb--ns tb--tar">Ganar</div>
                            </div>
                            <div className="table-height tb--flex-box">
                                <div className="tb--game-history">
                                    {loading ? (
                                        <div className="pt-3">
                                            <LoadApi />
                                        </div>
                                    ) : transactions.length > 0 ? (
                                        <div className="transaction-table transaction-table--generic-table casino-transaction-table">
                                            <div className="transaction-table__body-container">
                                                {transactions.map((txn, index) => (
                                                    <div className="transaction-table__row" key={index}>
                                                        <span className="transaction-table__cell">{index + 1}</span>
                                                        ...
                                                        <span className="transaction-table__cell justify-content-md-end">{formatBalance(txn.value_after)}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ) : <div className="tb--history-empty tb--flex tb--align-center tb--justify-center f-h-100 tb--mt-24">
                                        <div className="tb--empty-cont tb--tac">
                                            <div className="tb--nodata-icon">
                                                <NoDataSvg />
                                            </div>
                                            <span>Sin datos</span>
                                            <p>No hay datos para este período de tiempo</p>
                                        </div>
                                    </div>
                                    }
